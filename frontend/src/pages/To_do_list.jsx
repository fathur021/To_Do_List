      ...
      <div className="flex min-h-screen items-center justify-center">
        <div className="bg-white p-6 shadow-lg w-90 rounded-lg">
          ...
          <div className="flex gap-2 mt-10">
            ...
            <button onClick={addTodo} className="bg-blue-600 px-2 py-3 cursor-pointer rounded-2xl text-white hover:bg-blue-700">
              Tambah
            </button>
          </div>
          ...
          <div className="mt-6 space-y-2">
            {todos.length === 0 ? (
               ...
               <div key={todo.id} className="flex items-center justify-between bg-gray-200 p-3 rounded-lg">
              <div className="flex items-center gap-3">
                <input onClick={()=>toggleComplete(todo.id)} className="w-5 h-5" type="checkbox" />
                <spa>{todo.text}</spa>
              </div>
              <button 
              onClick={() => deleteTodo(todo.id)} className="text-red-400 hover:text-red-600 cursor-pointer">🗑️</button>
            </div> 
